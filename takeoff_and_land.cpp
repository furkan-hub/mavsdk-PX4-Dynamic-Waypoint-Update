//aisoft biyoteksan ship opperation mavsdk application
// 2024 Muhammed Furkan Doğan
...
void update_home(MavlinkPassthrough& mavlink_passthrough, double home_latitude, double home_longitude, float home_altitude) {
    ...
    command.param5 = home_latitude; // converting to int32 required by MAVLink
    command.param6 = home_longitude; // converting to int32 required by MAVLink
    command.param7 = home_altitude;

    mavlink_passthrough.send_command_long(command);

    cout << "Home noktası güncellendi: " << home_latitude << ", " << home_longitude << endl;
}

void update_waypoints(MissionRaw& mission_raw, vector<MissionRaw::MissionItem>& waypoints,vector<double> drone2_pos_vector,vector<double> drone1_homepos_vector) {
    ...
    //float bearing_radian = degree_to_radian(bearing);
    ...
    for (size_t i = 0; i < waypoints_coords.size(); ++i) {
        ...
        cout << "waypoint " << i << ": " << fixed << setprecision(8) <<point[0] << ", " << point[1] << endl;
        ...
        cout << "Güncellenmiş waypoint " << i << ": " << fixed << setprecision(8) << static_cast<int32_t>(new_coords[0] * 1e7) << ", " << static_cast<int32_t>(new_coords[1] * 1e7) << endl;
    }
    ...
    while (true) {
        ...
        if (distance_diff * 1000 >= distance_treshold) {
            ...
            if (mission_items_result.first == MissionRaw::Result::Success) {
                ...
                update_waypoints(mission_raw1, mission_items,drone2_pos_vector,drone1_homepos_vector); // Waypoint'leri güncelle
